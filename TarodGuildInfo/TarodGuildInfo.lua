...
function TarodGuildInfo:Initialize()
    ...
    TarodGuildInfo.chat:Print(zo_strformat("Welcome Back |cB27BFF<<1>>|r!", TarodGuildInfo.currentPlayer))
    zo_callLater(TarodGuildInfo.GuildInfo, 5000)
    TarodGuildInfo.alreadyLoggedIn = true
    LibChatMessage:SetTagPrefixMode(1)
    LibChatMessage:SetTimePrefixFormat(LibChatMessage.TIME_FORMATS[3])
end
...
function TarodGuildInfo:PrintGuildInfo(idx)
    ...
    TarodGuildInfo.chat:Print(zo_strformat("|cFFFFFF<<1>>|r", GetGuildName(guildId)))
    local onlineMemberCount = TarodGuildInfo:CountOtherGuildMembers(guildId)
    local additionalMembers = onlineMemberCount

    TarodGuildInfo.chat:Print(zo_strformat(" |cC3F0C2<<1[You are the only one online :(/There is only another member online:/There are $d members online:]>>|r", additionalMembers))
...
function TarodGuildInfo:PrintGuildMembers(guildId)
    ...
    for idx=1, guildMemberCount do
        ...
        if logoff == 0 and zo_strformat("<<1>>", charName) ~= TarodGuildInfo.currentPlayer then
            ...
            TarodGuildInfo.chat:Print(text)
        end
    end
end
...
SLASH_COMMANDS["/guildinfo"] = function (extra)
    ...
    elseif index >= 1 and index <= guilds then 
        ...
        TarodGuildInfo.chat:Print(zo_strformat("Please use |cC3F0C2/guildinfo|r |cB27BFF#num_guild|r where |cB27BFF#num_guild|r is a valid guild number between 1 and <<1>>", guilds))
        TarodGuildInfo.chat:Print("You can also use plain |cC3F0C2/guildinfo|r to get the default welcome message.")
    end
end
