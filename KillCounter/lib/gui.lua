...
function KC_G.statsWindowGUISetup()
   ...
   --]]
    ...
    for i = 1, tkw.MaxLines do
        tkw.Lines[i] = WINDOW_MANAGER:CreateControlFromVirtual(
                           "KillCounter_Kills_Table_Line_" .. i, tkw,
    ...
    for i = 1, tsw.MaxLines do
        tsw.Lines[i] = WINDOW_MANAGER:CreateControlFromVirtual(
                           "KillCounter_Seige_Overview_Line_" .. i, tsw,
                           "KillCounter_Overview_Line")
        ...
        for j = 1, tsw.MaxColumns do
            tsw.Lines[i].Columns[j] = WINDOW_MANAGER:CreateControl(nil,
                                                                   tsw.Lines[i],
                                                                   CT_LABEL)
            tsw.Lines[i].Columns[j]:SetFont("ZoFontGame")
            tsw.Lines[i].Columns[j]:SetDimensions(tsw.Lines[i]:GetWidth() / 3,
                                                  30)
            if j == 1 then
                tsw.Lines[i].Columns[j]:SetAnchor(TOPLEFT, tsw.Lines[i],
                                                  TOPLEFT, 18, 0)
            else
                tsw.Lines[i].Columns[j]:SetAnchor(TOPLEFT,
                                                  tsw.Lines[i].Columns[j - 1],
                                                  TOPLEFT,
                                                  (tsw.Lines[i]:GetWidth() / 3),
                                                  0)
            end
    ...
    local tex = "/esoui/art/miscellaneous/scrollbox_elevator.dds"
    tdw.Slider = WINDOW_MANAGER:CreateControl(
                     "KillCounter_THIS_FUCKING_ASSHOLE_SCROLLBAR_PART_2_BITCH",
                     tdw, CT_SLIDER)
    ...
    for i = 1, tdw.MaxLines do
        tdw.Lines[i] = WINDOW_MANAGER:CreateControlFromVirtual(
                           "KillCounter_Deaths_Table_Line_" .. i, tdw,
                           "KillCounter_Kills_Table_Line")
        ...
        for j = 1, tdw.MaxColumns do
            ...
            if i == 1 then
                ...
                tdw.Lines[i].Columns[j]:SetAnchor(TOPLEFT,
                                                  tdw.Lines[i - 1].Columns[j],
                                                  BOTTOMLEFT, offx, oy)
            end
            local function SortBy(sortField)
   ...
   --]]
    ...
    -- Templar Percentage
    tbgw.Templar = WINDOW_MANAGER:CreateControlFromVirtual(
                       "KillCounter_Breakdown_Graph_Templar", tbgw,
                       "KillCounter_Graph_Line_Templar")
    ...
    tbgw.DragonKnight = WINDOW_MANAGER:CreateControlFromVirtual(
                            "KillCounter_Breakdown_Graph_DragonKnight", tbgw,
                            "KillCounter_Graph_Line_DragonKnight")
    ...
    tbgw.NightBlade = WINDOW_MANAGER:CreateControlFromVirtual(
                          "KillCounter_Breakdown_Graph_NightBlade", tbgw,
                          "KillCounter_Graph_Line_NightBlade")
    ...
    -- Sorcerer Percentage
    tbgw.Sorcerer = WINDOW_MANAGER:CreateControlFromVirtual(
                        "KillCounter_Breakdown_Graph_Sorcerer", tbgw,
                        "KillCounter_Graph_Line_Sorcerer")
    ...
    -- Warden Percentage
    tbgw.Warden = WINDOW_MANAGER:CreateControlFromVirtual(
                      "KillCounter_Breakdown_Graph_Warden", tbgw,
                      "KillCounter_Graph_Line_Warden")
    ...
    tbgw.Necromancer = WINDOW_MANAGER:CreateControlFromVirtual(
                           "KillCounter_Breakdown_Graph_Necromancer", tbgw,
                           "KillCounter_Graph_Line_Necromancer")
    tbgw.Necromancer:SetAnchor(TOPLEFT, tbgw.Warden, BOTTOMLEFT, 0, 3)

    tbgw.Necromancer.Label = WINDOW_MANAGER:CreateControl(nil, tbgw.Necromancer,
                                                          CT_LABEL)
    ...
    -- Templar Percentage
    tbdgw.Templar = WINDOW_MANAGER:CreateControlFromVirtual(
                        "KillCounter_Breakdown_Death_Graph_Templar", tbdgw,
                        "KillCounter_Graph_Line_Templar")
    tbdgw.Templar:SetAnchor(TOPLEFT, tbdgw, TOPLEFT, 5, 5)

    tbdgw.Templar.Label = WINDOW_MANAGER:CreateControl(nil, tbdgw.Templar,
                                                       CT_LABEL)
    ...
    -- Dragon Knight Percentage
    tbdgw.DragonKnight = WINDOW_MANAGER:CreateControlFromVirtual(
                             "KillCounter_Breakdown_Death_Graph_DragonKnight",
                             tbdgw, "KillCounter_Graph_Line_DragonKnight")
    tbdgw.DragonKnight:SetAnchor(TOPLEFT, tbdgw.Templar, BOTTOMLEFT, 0, 3)

    tbdgw.DragonKnight.Label = WINDOW_MANAGER:CreateControl(nil,
                                                            tbdgw.DragonKnight,
                                                            CT_LABEL)
    tbdgw.DragonKnight.Label:SetFont("ZoFontGame")
    tbdgw.DragonKnight.Label:SetDimensions(tbdgw:GetWidth(), 30)
    tbdgw.DragonKnight.Label:SetAnchor(TOPLEFT, tbdgw.DragonKnight, TOPLEFT, 0,
                                       2)

    -- NightBlade Percentage
    tbdgw.NightBlade = WINDOW_MANAGER:CreateControlFromVirtual(
                           "KillCounter_Breakdown_Death_Graph_NightBlade",
                           tbdgw, "KillCounter_Graph_Line_NightBlade")
    tbdgw.NightBlade:SetAnchor(TOPLEFT, tbdgw.DragonKnight, BOTTOMLEFT, 0, 3)

    tbdgw.NightBlade.Label = WINDOW_MANAGER:CreateControl(nil, tbdgw.NightBlade,
                                                          CT_LABEL)
    ...
    -- Sorcerer Percentage
    tbdgw.Sorcerer = WINDOW_MANAGER:CreateControlFromVirtual(
                         "KillCounter_Breakdown_Death_Graph_Sorcerer", tbdgw,
                         "KillCounter_Graph_Line_Sorcerer")
    tbdgw.Sorcerer:SetAnchor(TOPLEFT, tbdgw.NightBlade, BOTTOMLEFT, 0, 3)

    tbdgw.Sorcerer.Label = WINDOW_MANAGER:CreateControl(nil, tbdgw.Sorcerer,
                                                        CT_LABEL)
    ...
    -- Warden Percentage
    tbdgw.Warden = WINDOW_MANAGER:CreateControlFromVirtual(
                       "KillCounter_Breakdown_Death_Graph_Warden", tbdgw,
                       "KillCounter_Graph_Line_Warden")
    tbdgw.Warden:SetAnchor(TOPLEFT, tbdgw.Sorcerer, BOTTOMLEFT, 0, 3)

    tbdgw.Warden.Label = WINDOW_MANAGER:CreateControl(nil, tbdgw.Warden,
                                                      CT_LABEL)
    ...
    -- Necromancer Percentage
    tbdgw.Necromancer = WINDOW_MANAGER:CreateControlFromVirtual(
                            "KillCounter_Breakdown_Death_Graph_Necromancer",
                            tbdgw, "KillCounter_Graph_Line_Necromancer")
    tbdgw.Necromancer:SetAnchor(TOPLEFT, tbdgw.Warden, BOTTOMLEFT, 0, 3)

    tbdgw.Necromancer.Label = WINDOW_MANAGER:CreateControl(nil,
                                                           tbdgw.Necromancer,
                                                           CT_LABEL)
...
function KC_G.UpdateGui()
    ...
    if showingStats and KC_G.savedVars ~= nil and stats_table ~= nil then
        ...
        for i = 1, breakdown_table.MaxLines do
            for j = 1, breakdown_table.MaxColumns do
                if i == 1 then
                    ...
                    if i == 5 then
                        if j == 2 then
                            local temp, sorc, dk, nb, ward, necro = 0, 0, 0, 0,
                                                                    0, 0
                            for ii, v in pairs(KC_G.savedVars.players) do
                            ...
                            local max =
                                math.max(temp, sorc, dk, nb, ward, necro)
                            if temp == max then
                                breakdown_table.Lines[i].Columns[j]:SetText(
                                    zo_strformat("<<1>>", GetClassName(
                                                     GENDER_MALE,
                                                     KC_CLASS_TEMPLAR)) .. " (" ..
                                        max .. " Kills)")
                    ...
                    if i == 9 then
                        if j == 2 then
                            ...
                            local max = math.max(temp, sorc, dk, nb, ward)
                            if temp == max then
                                breakdown_table.Lines[i].Columns[j]:SetText(
                                    zo_strformat("<<1>>", GetClassName(
                                                     GENDER_MALE,
        ...
        -- Necromancer kills
        breakdown_graph.Necromancer.Label:SetText(
            string.format("|Cababab%s: %.2f%% %d Kills|r",
                          zo_strformat("<<1>>", GetClassName(GENDER_MALE,
                                                             KC_CLASS_NECROMANCER)),
                          killList.necrop * 100, killList[KC_CLASS_NECROMANCER]))
        breakdown_graph.Necromancer:SetDimensions(
            ((breakdown_graph:GetWidth() * .75) - 10) * killList.necrop, 30)

        -- Warden kills
        breakdown_graph.Warden.Label:SetText(
            string.format("|Cababab%s: %.2f%% %d Kills|r",
                          zo_strformat("<<1>>", GetClassName(GENDER_MALE,
                                                             KC_CLASS_WARDEN)),
                          killList.wardp * 100, killList[KC_CLASS_WARDEN]))
        breakdown_graph.Warden:SetDimensions(
            ((breakdown_graph:GetWidth() * .75) - 10) * killList.wardp, 30)

        -- DragonKnight kills
        breakdown_graph.DragonKnight.Label:SetText(
            string.format("|Cababab%s: %.2f%% %d Kills|r",
                          zo_strformat("<<1>>", GetClassName(GENDER_MALE,
                                                             KC_CLASS_DRAGONKNIGHT)),
                          killList.dkp * 100, killList[KC_CLASS_DRAGONKNIGHT]))
        breakdown_graph.DragonKnight:SetDimensions(
            ((breakdown_graph:GetWidth() * .75) - 10) * killList.dkp, 30)

        -- Sorcerer kills
        breakdown_graph.Sorcerer.Label:SetText(
            string.format("|Cababab%s: %.2f%% %d Kills|r",
                          zo_strformat("<<1>>", GetClassName(GENDER_MALE,
                                                             KC_CLASS_SORCERER)),
                          killList.sorcp * 100, killList[KC_CLASS_SORCERER]))
        breakdown_graph.Sorcerer:SetDimensions(
            ((breakdown_graph:GetWidth() * .75) - 10) * killList.sorcp, 30)

        -- Nightblade kills

        breakdown_graph.NightBlade.Label:SetText(
            string.format("|Cababab%s: %.2f%% %d Kills|r",
                          zo_strformat("<<1>>", GetClassName(GENDER_MALE,
                                                             KC_CLASS_NIGHTBLADE)),
                          killList.nbp * 100, killList[KC_CLASS_NIGHTBLADE]))

        breakdown_graph.NightBlade:SetDimensions(
            ((breakdown_graph:GetWidth() * .75) - 10) * killList.nbp, 30)

        -- Templar kills
        breakdown_graph.Templar.Label:SetText(
            string.format("|Cababab%s: %.2f%% %d Kills|r",
                          zo_strformat("<<1>>", GetClassName(GENDER_MALE,
                                                             KC_CLASS_TEMPLAR)),
                          killList.tempp * 100, killList[KC_CLASS_TEMPLAR]))
        breakdown_graph.Templar:SetDimensions(
            ((breakdown_graph:GetWidth() * .75) - 10) * killList.tempp, 30)
        ...
        breakdown_death_graph.Necromancer.Label:SetText(
            string.format("|Cababab%s: %.2f%% %d Deaths|r", zo_strformat(
                              "<<1>>",
                              GetClassName(GENDER_MALE, KC_CLASS_NECROMANCER)),
                          deathList.necrop * 100,
                          deathList[KC_CLASS_NECROMANCER]))
        breakdown_death_graph.Necromancer:SetDimensions(
            ((breakdown_graph:GetWidth() * .75) - 10) * deathList.necrop, 30)
        -- Warden label

        breakdown_death_graph.Warden.Label:SetText(
            string.format("|Cababab%s: %.2f%% %d Deaths|r", zo_strformat(
                              "<<1>>",
                              GetClassName(GENDER_MALE, KC_CLASS_WARDEN)),
                          deathList.wardp * 100, deathList[KC_CLASS_WARDEN]))
        breakdown_death_graph.Warden:SetDimensions(
            ((breakdown_graph:GetWidth() * .75) - 10) * deathList.wardp, 30)

        -- DragonKnight label

        breakdown_death_graph.DragonKnight.Label:SetText(
            string.format("|Cababab%s: %.2f%% %d Deaths|r", zo_strformat(
                              "<<1>>",
                              GetClassName(GENDER_MALE, KC_CLASS_DRAGONKNIGHT)),
                          deathList.dkp * 100, deathList[KC_CLASS_DRAGONKNIGHT]))
        breakdown_death_graph.DragonKnight:SetDimensions(
            ((breakdown_graph:GetWidth() * .75) - 10) * deathList.dkp, 30)

        -- Sorcerer label

        breakdown_death_graph.Sorcerer.Label:SetText(
            string.format("|Cababab%s: %.2f%% %d Deaths|r", zo_strformat(
                              "<<1>>",
                              GetClassName(GENDER_MALE, KC_CLASS_SORCERER)),
                          deathList.sorcp * 100, deathList[KC_CLASS_SORCERER]))
        breakdown_death_graph.Sorcerer:SetDimensions(
            ((breakdown_graph:GetWidth() * .75) - 10) * deathList.sorcp, 30)

        -- Nightblade label

        breakdown_death_graph.NightBlade.Label:SetText(
            string.format("|Cababab%s: %.2f%% %d Deaths|r", zo_strformat(
                              "<<1>>",
                              GetClassName(GENDER_MALE, KC_CLASS_NIGHTBLADE)),
                          deathList.nbp * 100, deathList[KC_CLASS_NIGHTBLADE]))
        breakdown_death_graph.NightBlade:SetDimensions(
            ((breakdown_graph:GetWidth() * .75) - 10) * deathList.nbp, 30)

        -- Templar Label
        breakdown_death_graph.Templar.Label:SetText(
            string.format("|Cababab%s: %.2f%% %d Deaths|r", zo_strformat(
                              "<<1>>",
                              GetClassName(GENDER_MALE, KC_CLASS_TEMPLAR)),
                          deathList.tempp * 100, deathList[KC_CLASS_TEMPLAR]))
        breakdown_death_graph.Templar:SetDimensions(
            ((breakdown_graph:GetWidth() * .75) - 10) * deathList.tempp, 30)
    end
end
