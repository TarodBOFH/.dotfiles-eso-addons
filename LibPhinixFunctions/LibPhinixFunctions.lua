...
function lib.LangFormat(langText) -- ZOS function for removing special characters from names in various languages.
	-- langText:	String, text to remove language markup (masculine, feminine, etc.).

	if langText == nil then
		if enableDebug then
			d("LibPhinixFunctions: No text passed to LangFormat function.")
		end
		return
	end
